...
export class QuestionGenerator {
  ...
  private filterByMode(
    ...
    switch (mode) {
      ...
      case "asking":
        ...
        return questions.filter((q) =>
          ["user-experience", "business-logic", "architecture"].includes(q.category)
        );
      default:
        return questions;
    }
  ...
  private generateArchitectureQuestions(context: ProjectContext): StrategicQuestion[] {
    ...
    if (context.projectType === "monorepo") {
      questions.push({
        ...
        context:
          "Monorepo architecture decisions early prevent painful refactors later",
        options: [
  ...
  private generateScalabilityQuestions(context: ProjectContext): StrategicQuestion[] {
    const questions: StrategicQuestion[] = [];
  ...
  private generateUserExperienceQuestions(
    ...
    if (context.hasFrontend) {
      ...
      questions.push({
        ...
        context: "Poor onboarding is the #1 cause of churn for SaaS products",
        options: [
  ...
  private generateSecurityQuestions(context: ProjectContext): StrategicQuestion[] {
    const questions: StrategicQuestion[] = [];
    ...
    if (context.hasAuthentication) {
      questions.push({
        ...
        context: "MFA prevents 99.9% of automated account compromise attacks",
        options: [
  ...
  private generateBusinessLogicQuestions(context: ProjectContext): StrategicQuestion[] {
    const questions: StrategicQuestion[] = [];
  ...
  private generateTechnicalDebtQuestions(context: ProjectContext): StrategicQuestion[] {
    const questions: StrategicQuestion[] = [];
  ...
  private generateDeploymentQuestions(context: ProjectContext): StrategicQuestion[] {
    const questions: StrategicQuestion[] = [];
  ...
  private generateDataManagementQuestions(
    ...
    if (context.hasDatabase) {
      ...
      questions.push({
        ...
        context: "Botched migrations are a top cause of production outages",
        options: [
